...
negflag         RESB    1              ;P=positive, N=negative	
	...
	;Start program code.
section .text	;for elf linker (extended linking format
	global main
main:	nop
	; prompt user for positive number	
Again:  call    PrintString		; read Y
call   GetAnInteger ;get an integer, converts to binary.

	;mov     ax,[ReadInt]  ; Loop if non-zero
	;cmp     ax,0       ;Terminate if zero.
	;je      fini                    

	mov ax,[ReadInt]
	mov [Y],ax		

	call PrintString		; read Z
	call GetAnInteger
	mov ax,[ReadInt]
	mov [Z],ax
	...
	mov ax,[T2]		;T3 = (M + (Y * z) ) + Lit12
	add ax,[Lit12]
	mov [T3],ax

	mov ax,[T3]		;X = (M + (Y * z) ) + Lit12
	mov [X], ax
